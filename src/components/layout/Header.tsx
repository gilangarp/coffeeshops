  ...
  return (
    <header className="py-2 z-50 px-5 md:px-10 lg:px-14 bg-black bg-opacity-80 shadow-md w-full fixed top-0 left-0 lg:flex lg:justify-between lg:items-center">
      ...
      <nav>
        ...
        {/* mobile */}
        <div className={`grid gap-4 py-4 lg:items-center w-full transition-all ease-in duration-500 ${isOpen ? "hidden" :"block"} lg:hidden `}>
          {/* navigation links */}
          {Page.map((link) => (
            <Link className="text-sm flex gap-4 items-center list-none text-white" to={link.link}>
              {link.name}
            </Link>
          ))}
